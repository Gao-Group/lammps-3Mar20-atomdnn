  ...
  pack_choice(NULL)
{
  ...
  // added to allow 'compute fingerprnts' to compute both peratom and local
  int iarg,icompute;
  for (iarg = 0; iarg < narg; iarg++){
    if (strncmp(arg[iarg],"c_",2) == 0){
      char *comp_keyword;
      int len = sizeof(arg[iarg])/sizeof(arg[iarg][0]);
      comp_keyword = new char[len];
      for (int i=0;i<len;i++)
	comp_keyword[i]= arg[iarg][i];
