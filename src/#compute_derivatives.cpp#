  ...
  const int* const numneigh = list->numneigh;
    ...
    for (int ii = 0; ii < inum; ii++) {
      ...
      if (mask[i] & groupbit) {
          ...
          if (rsq < cutsq && rsq>1e-20) { 
            ...
            if (g2_flag == 1) {
              ...
              for (int m = 0; m < n_etaG2; m++)  {
                ...
                derivatives_i[1][ntypes+m*ntypes+jtype] += -exp(-eta_G2[m]*rsq)*(dfc/sqrt(rsq)-2*eta_G2[m]*function)*Rx_ij; // G2 derivatives in the x direction
                derivatives_i[2][ntypes+m*ntypes+jtype] += -exp(-eta_G2[m]*rsq)*(dfc/sqrt(rsq)-2*eta_G2[m]*function)*Ry_ij; // G2 derivatives in the y direction
                derivatives_i[3][ntypes+m*ntypes+jtype] += -exp(-eta_G2[m]*rsq)*(dfc/sqrt(rsq)-2*eta_G2[m]*function)*Rz_ij; // G2 derivatives in the z direction 
              }
            }
            ...
            for(int n = 0; n < size_local_cols; n++) {
              alocal[0+count*4][n] = derivatives[0][n];
              alocal[1+count*4][n] = derivatives[1][n];
              alocal[2+count*4][n] = derivatives[2][n];
              alocal[3+count*4][n] = derivatives[3][n];
            }
            count++;
          }
        }
        ...
        for(int n = 0; n < size_local_cols; n++) {
          alocal[0+count*4][n] = derivatives_i[0][n];
          alocal[1+count*4][n] = derivatives_i[1][n];
          alocal[2+count*4][n] = derivatives_i[2][n];
          alocal[3+count*4][n] = derivatives_i[3][n];
        }
        count++;
      } 
    }
