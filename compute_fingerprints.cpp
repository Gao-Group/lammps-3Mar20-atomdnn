...
void ComputeFingerprints::compute_local()
{
...
int ComputeFingerprints::compute_derivatives(int flag)
{
  const int inum = list->inum;
  const int* const numneigh = list->numneigh;
  ...
  if (flag) {
    ...
    // Initialize fingerprnts and fingerprints_i array
    double fingerprnts[4][size_local_cols];
    double fingerprnts_i[4][size_local_cols];
...
void ComputeFingerprints::reallocate(int n)
{
  while (nmax_local < n) nmax_local += 1;

  memory->destroy(alocal);
  memory->create(alocal,nmax_local,size_local_cols,"fingerprints:array_local");
  array_local = alocal;
}
...
{
  double bytes = (size_peratom_cols*nmax_atom+size_local_cols*nmax_local) * sizeof(double);
  return bytes;
}
